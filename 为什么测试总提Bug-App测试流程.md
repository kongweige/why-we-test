...
对于开发来说拿到一个从测试角度来说拿到一个app（或某个功能模块）的需求后要进行开发测试流程分为下面几个步骤：
...
2. 应用程序第一次启动速度慢
3. 进入到某一个界面加载速度慢
4. 启动某一个有动画效果的界面，动画执行过程加载度慢并且有卡顿
5. 响应某一个用户事件时，长时间无响应（ANR）

* 其他
1. 太占用手机内存
2. 太耗电和流量
3. 在使用应用的过程中，点击某一个事件进入的页面现白屏的闪动等

## bug处理流程
![alt text](./images/bug处理流程.png)

## 基于模型测试
最近在企业落地ai相关的自动化测试流程，发现原本只是作为探索性的MBT变成了ai感知世界的基石，像前端UI的自动化agent在当时落地第一版的时候，测试用例的编写像这种形式：1. 进入搜索界面；2. 点击右上角的搜索框；3. 输入"测试"；4. 点击回车；这种方法的缺点就是当界面有一点变动测试用例就需要跟着改变要不然就会执行失败，所以现在优化的方向就是用例只按照PO建模的形式编写，剩下的具体细节步骤让ai自己去搜索，通过爬虫爬下来待测产品的Doc结构，转成一个正确的图谱，执行测试极大程度避免了需要人为手工不断优化用例和图谱的成本。（目前也还在探索中后续有时间单独把AI相关的内容整理一下）
